...
fields = {
  ...
  'median_score': lambda x: float(get_number(x.get('median_score', {'N': 0}))),
...
if X is None and y is None:
  ...
  while has_more:
    if last_evaluated_key:
      results = client.scan(TableName="automod_events_production", Limit=limit, ExclusiveStartKey=last_evaluated_key)
    else:
      results = client.scan(TableName="automod_events_production", Limit=limit)
